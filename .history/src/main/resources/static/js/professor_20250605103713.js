...
function editGrade(enrollmentId, studentName, courseName) {
  ...
  fetch(`/professor/api/grades/enrollment/${enrollmentId}`)
    ...
    .then((grades) => {
        ...
        <div class="grades-list">
          ...
          ${grades
            ...
            <div class="grade-edit-item">
              <form id="editGradeForm_${grade.id}" class="edit-grade-form">
                <input type="hidden" name="_csrf" value="${csrfToken}">
                <div class="form-group">
                  <label>Assignment Name</label>
                  <input type="text" name="assignmentName" value="${grade.assignmentName}" required>
                </div>
                <div class="form-group">
                  <label>Type</label>
                ...
                <div class="form-group">
                  <label>Score</label>
                  <input type="number" name="score" value="${grade.score}" min="0" max="100" step="0.1" required>
                </div>
                <div class="form-group">
                  <label>Comments</label>
                  <textarea name="comments">${grade.comments || ""}</textarea>
                </div>
      ...
      // Setup form handlers for each grade edit form
      grades.forEach((grade) => {
        const editForm = document.getElementById(`editGradeForm_${grade.id}`)
        if (editForm) {
          ...
          fetch("/professor/api/grades", {
            ...
            .then((response) => {
              if (response.ok) {
                ...
                // Refresh the grades list in the modal
                editGrade(enrollmentId, studentName, courseName)
              } else {
                return response.text().then((text) => {
                  throw new Error(text)
                })
    ...
    fetch(`/professor/api/grades/${gradeId}`, {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          // Don't close modal, just refresh the search results
          searchGrades()
        } else {
...
function closeModal() {
  ...
  if (modal) {
    ...
    // Refresh the search results when closing the modal
    searchGrades()
  }
}
