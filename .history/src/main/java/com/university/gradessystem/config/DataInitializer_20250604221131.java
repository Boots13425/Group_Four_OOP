...
public class DataInitializer implements CommandLineRunner {
    ...
    private void initializeCourses() {
        // Get users by username and cast properly
        User prof1 = userService.getUsersByRole(User.Role.ROLE_PROFESSOR).stream()
                .filter(u -> u.getUsername().equals("prof1"))
                .findFirst().orElse(null);

        User prof2 = userService.getUsersByRole(User.Role.ROLE_PROFESSOR).stream()
                .filter(u -> u.getUsername().equals("prof2"))
                .findFirst().orElse(null);
    ...
    private void initializeEnrollments() {
        if (enrollmentService.getAllEnrollments().isEmpty()) {
            // Get students by role and find specific ones
            List<User> students = userService.getUsersByRole(User.Role.ROLE_STUDENT);
            User student1 = students.stream().filter(u -> u.getUsername().equals("student1")).findFirst().orElse(null);
            User student2 = students.stream().filter(u -> u.getUsername().equals("student2")).findFirst().orElse(null);
            User student3 = students.stream().filter(u -> u.getUsername().equals("student3")).findFirst().orElse(null);
